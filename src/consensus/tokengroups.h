...
public:
    ...
    CTokenGroupInfo(const CTokenGroupID &associated, const GroupControllerFlags _controllingGroupFlags, CAmount qty = 0)
        : associatedGroup(associated), controllingGroupFlags(_controllingGroupFlags), quantity(qty), invalid(false)
    {
    }
    CTokenGroupInfo(const CKeyID &associated, const GroupControllerFlags _controllingGroupFlags, CAmount qty = 0)
        : associatedGroup(associated), controllingGroupFlags(_controllingGroupFlags), quantity(qty), invalid(false)
    {
    }
